...
player_df <- pbp_df %>% 
  ...
  summarise(n = n(), mean_air_yards = mean(air_yards), sum_air_yards = sum(air_yards)) %>% 
  arrange(-n) %>% 
...
ggsave('week5only air yards team panel sorted targets 20225.png', width = 10, height = 14, dpi = "retina")
