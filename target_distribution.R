# couple issues
# if pbp data and roster data are not in the same week
# will return error messages. lets say a thursday night game has already been played, 
# week 5 will be displayed but player data is only available for week 4, script doesnt pick up on that
# leaving out week from roster fixes the issue
# saving the file displays warnings, can ignore.
library(ggplot2)
library(gridExtra)
library(grid)
library(png)
...
player_df <- pbp_df %>% 
  ...
  #arrange(-mean_air_yards) %>%   #uncomment to sort by avg depth of targets
...
tm_grob_df <- data.frame(posteam = .tm_div_order, team_air_rank = 2, full_name = NA, headshot_url = ESPN_logo_url(.tm_div_order), air_yards = 20, 
                         vp.height = 1, alpha = 0.4)
# below part is only used for geom_image headshots
grob_df <- player_df %>% 
...
p <- pbp_df %>% 
  ...
  geom_density_ridges(aes(color = posteam, fill = posteam), quantile_lines = TRUE, quantiles = 0.5, scale = 1.2, bandwidth = 2, panel_scaling = F, show.legend = F, size = 0.4) +
  ...
  labs(title = '2022 Targeted Air Yards Distribution',
       subtitle = paste0('Four most targeted players on each team through Week ', my_week, ', sorted by # of targets'),
       x = NULL,
       y = NULL) +
  theme_SB +
  theme(
...
ggsave('target_distribution_2022_season.png', width = 10, height = 14, dpi = "retina")
